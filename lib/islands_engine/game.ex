defmodule IslandsEngine.Game do
  ...
  def guess_coordinate(game, player, row, col) when valid_player?(player) do
    GenServer.call(game, {:guess_coordinate, player, row, col})
  end

  def position_island(game, player, key, row, col) when valid_player?(player) do
    GenServer.call(game, {:position_island, player, key, row, col})
  end

  def set_islands(game, player) when valid_player?(player) do
  ...
  def terminate(_event, _state_data), do: :ok
  ...
  def handle_call({:add_player, name}, _from, state_data) do
    with {:ok, rules} <- Rules.check(state_data.rules, :add_player) do
      ...
      :error -> {:reply, {:error, "Only 2 players allowed"}, state_data}
    end
  end
  ...
  defp player_board(state_data, player), do: get_in(state_data, [player, :board])
  ...
  defp update_board(state_data, player, board) do
    update_in(state_data[player], &Map.put(&1, :board, board))
  end

  defp update_guesses(state_data, player, hit_or_miss, coordinate) do
    update_in(state_data[player].guesses, &Guesses.add(&1, hit_or_miss, coordinate))
  end
